...
def _make_public_product_name(analyzed: Dict) -> str:
    ...
    suggested = str(analyzed.get("suggested_name") or "").strip()
    rule_corrected = str(analyzed.get("rule_corrected") or "").strip()
    display_name = str(analyzed.get("display_name") or "").strip()
    confidence = float(analyzed.get("match_confidence", 0) or 0)

    if corrected:
        return corrected
    if confidence >= 0.80 and suggested:
        return suggested
    if confidence >= 0.86 and rule_corrected:
        return rule_corrected
    if confidence >= 0.90 and display_name:
        return display_name
    return ""
...
def _save_user_medicine(
    ...
    cursor.execute(
        """
        SELECT id, COALESCE(active_ingredients, ''), COALESCE(source_type, 'manual'), COALESCE(ocr_text, '')
        FROM user_medicines
        WHERE nickname = ? AND medicine_name = ?
        ORDER BY id DESC
        LIMIT 1
        """,
    ...
    if not exists:
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (nickname, medicine_name, ingredients_text, source_type, ocr_text),
        )
        inserted = True
    else:
        row_id, old_ingredients_text, old_source_type, old_ocr_text = exists
        merged_ingredients = _merge_ingredient_lists([_split_ingredient_text(old_ingredients_text), new_ingredients])
        next_ingredients_text = _join_ingredients_text(merged_ingredients)
        next_source_type = old_source_type or source_type
        next_ocr_text = old_ocr_text or ocr_text
        cursor.execute(
            """
            UPDATE user_medicines
            SET active_ingredients = ?, source_type = ?, ocr_text = ?
            WHERE id = ?
            """,
            (next_ingredients_text, next_source_type, next_ocr_text, row_id),
        )
...
def extract_active_ingredients(raw_text: str) -> List[str]:
    """OCR 텍스트에서 유효성분만 보수적으로 추출하되, 앵커 주변 여러 줄을 함께 본다."""
    text = str(raw_text or "").replace("\r", "\n")
    if not text.strip():
        return []
...
def _pick_confirmed_name(
    ...
    """제품명 자동 확정만 허용하고, 성분명으로 제품명을 억지 확정하지 않는다."""
...
def _analyze_drug_text(raw_text: str) -> Dict:
    ...
    active_ingredients = _merge_ingredient_lists([extract_active_ingredients(raw_text)])
    rule_corrected = correct_drug_name(raw_text)

    lookup_seed = rule_corrected or raw_text
    resolved = resolve_drug_name(lookup_seed)
    candidate_resolved = _resolve_best_from_name_candidates(raw_text, rule_corrected, lookup_seed)
    if _resolved_rank(candidate_resolved) > _resolved_rank(resolved):
        resolved = candidate_resolved

    picked = _pick_confirmed_name(resolved, active_ingredients)
    final_resolved = picked["resolved"]
    confirmed_name = picked["confirmed_name"]
    product_name_candidates = _extract_product_name_candidates_from_text(raw_text)

    catalog_hit = _catalog_enrich_from_candidates(
        raw_text,
        active_ingredients,
        confirmed_name,
        (final_resolved.get("resolved_name") if final_resolved.get("auto_applied") else "") or "",
        rule_corrected,
    )
    ...
    display_name = (
        confirmed_name
        or (catalog_hit.get("display_name") or "")
        or final_resolved.get("resolved_name")
        or rule_corrected
        or final_resolved.get("suggested_name")
        or (product_name_candidates[0] if product_name_candidates else "")
        or (raw_text[:80] if raw_text else "")
    )

    corrected_name = confirmed_name or (catalog_hit.get("display_name") or "")
    ...
    analyzed = {
        ...
        "match_candidates": final_resolved.get("candidates", []),
        "match_note": match_note,
        "auto_applied": bool(corrected_name),
        "needs_confirm": not bool(corrected_name),
        "suggested_name": corrected_name or final_resolved.get("suggested_name") or "",
...
async def analyze_drug(
    ...
    return {
        ...
        "corrected_name": new_drug.get("corrected_name") or new_drug.get("display_name") or "",
